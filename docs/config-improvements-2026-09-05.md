...
| Complete laptop disk-encryption plan | Protect data when the laptop is powered off | Follow `docs/runbooks/bandit-luks-in-place.md`; in-place `cryptsetup reencrypt` (no reinstall), requires verified backups and a separately authorized maintenance window. |
